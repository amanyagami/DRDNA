...
def test_with_fault(model, tau1, tau2, tau3):
    ...
    with open('/home/local/ASUAD/asing651/ResnetCifar10pytorchFI/OODResults/ODDSVHNoutput.csv', 'w') as file:
        pass
    with open('/home/local/ASUAD/asing651/ResnetCifar10pytorchFI/OODResults/ODDSVHNoutput.csv',  mode='a', newline='') as file:
        writer = csv.writer(file)
        writer.writerow(["layer,Tau1, Tau2, Tau3 , T[-1]"])
    with torch.no_grad():
        for batch_idx, (inputs, targets) in enumerate(testloader):
            ...
            with open('/home/local/ASUAD/asing651/ResnetCifar10pytorchFI/OODResults/ODDSVHNoutput.csv', mode='a', newline='') as file:
                ...
                for layer in LayerNames:
                    ...
                    writer.writerow(["layer",TAU1[layer],TAU2[layer],TAU3[layer]])
                
            
            
            # flag denotes only those inputs which are not masked by the model resulting in SDC
            
            with open('OODSVHNoutput.csv', mode='a', newline='') as file:
                writer = csv.writer(file)
...
if __name__ == '__main__':
    ...
    svhn_testset = torchvision.datasets.SVHN(root='./data', split='test', download=True, transform=transform_test)
    testloader = torch.utils.data.DataLoader(svhn_testset, batch_size=1, shuffle=False, num_workers=2)
